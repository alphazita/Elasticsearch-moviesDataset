...
df_clusters = pd.read_csv(r".\data\df_clusters.csv",engine="python")
df_mov_clst = pd.read_csv(r".\data\ratings_movies_clusters.csv",engine="python")
...
while (temp != 0):
    ...
    if hits != 0:
        for hit in res['hits']['hits']:
            ...
            cluster = str(int(df_clusters.loc[user].values))
            if (usrrat.size == 0) :
                usrrat = float(df_mov_clst.loc[df_mov_clst['movieId'] == movie, cluster].values)
            
            #Extracting the final rating score while checking if user and average ratings exist
            if (math.isnan(usrrat)) :
